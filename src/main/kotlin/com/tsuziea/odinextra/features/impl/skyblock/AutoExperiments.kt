...
object AutoExperiments : Module(
    ...
    private class ChronomatronHandler : ExperimentHandler() {
        ...
        override fun onSlotUpdate(event: GuiEvent.SlotUpdate) {
            ...
            if (lastAddedSlot != -1 && center.item == Items.GLOWSTONE && !slots[lastAddedSlot].item.hasGlint()) {
    ...
    private class UltrasequencerHandler : ExperimentHandler() {
        ...
        override fun onSlotUpdate(event: GuiEvent.SlotUpdate) {
            ...
            for (slot in slots) {
                if (slot.index in 9..44 && slot.item.hoverName.string.noControlCodes.matches(Regex("\\d+"))) order[slot.item.count - 1] =
                    slot.index
            }
        ...
        override fun shouldClose(autoClose: Boolean): Boolean =
            autoClose && order.size > if (getMaxXp) 20 else 9 - serumCount
    }
    ...
    private fun delay(): Long = (clickDelay + (0..delayVariety).random()).toLong()
}